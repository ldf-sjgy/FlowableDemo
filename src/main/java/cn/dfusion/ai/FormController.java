...
public class FormController {
    ...
    @GetMapping
    public String getForm(@RequestParam(defaultValue = "form1") String formId) {
        ...
        return info.getFormModel().toString();
    }

    @PostMapping
    public String start(String formId, String outcome, HttpServletRequest request) {
        ProcessDefinition pd = repositoryService.createProcessDefinitionQuery().processDefinitionKey(formId)
                .latestVersion().singleResult();
        StartFormData form = formService.getStartFormData(pd.getId());
